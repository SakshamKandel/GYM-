...
/**
 * The shared status table narrowed to this client's union: the check-in card
 * has no separate copy for 403/404/409/429/503, so those keep reading as a
 * plain failure.
 */
function statusToCode(status: number): CheckInErrorCode {
...
async function request(opts: {
  ...
  if (!res.ok) throw new CheckInApiError(statusToCode(res.status));
