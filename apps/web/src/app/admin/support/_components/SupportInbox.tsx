 ...
 *
 * Opening a thread calls GET /api/admin/support/threads/[accountId], which
 * marks the account's inbound rows readByCoach=true server-side; closing the
 * drawer (or sending a reply) triggers router.refresh() so the table's unread
 * badges and stat tiles reflect that.
 */
...
function SupportThreadDrawer({
  ...
  async function send() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(res.status === 403 ? 'You do not have permission to reply.' : 'Could not send the reply.');
        return;
      }
      setBody('');
