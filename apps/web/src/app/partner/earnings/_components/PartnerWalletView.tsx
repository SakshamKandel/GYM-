 ...
 * lived on the retired /partner/wallet route until the two money pages were
 * merged). Renders the withdrawable balance, a request-payout form (or the
 * current pending request), and the payout + movement history. All money is
 * integer minor units; the form converts the major-unit input before POSTing.
 * The server re-validates everything (amount bound, one-pending) — this UI only
 * pre-flights for UX.
 *
 * Wording rule: this block states what the partner ACTUALLY receives. Cash taken
 * at the door never enters it — that money is already theirs.
 */
...
/** Ledger rows in the partner's words, not the table's. */
...
function errorMessage(code: string, currency: string, heldMinor: number): string {
  switch (code) {
    ...
    case 'already_pending':
      return 'You already have a request waiting for an answer.';
    default:
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <span style={{ fontSize: 12, color: 'var(--gt-text-faint)' }}>
          Digital payments only, cash at the door is already yours ·{' '}
          {formatMoney(earnedMinor, currency)} in, {formatMoney(paidOutMinor, currency)} paid out
        </span>
      </div>
      ...
      {initialPending ? (
        ...
        >
          ...
          <span style={{ fontSize: 13, color: 'var(--gt-text-dim)' }}>
            Asked for on {formatDate(initialPending.requestedAt)}. Someone from GM will check it and
            send the money. You can ask again once this one is answered.
          </span>
        </div>
      ) : (
        <form
        ...
        >
          ...
          {canRequest ? (
            ...
            <span style={{ fontSize: 13, color: 'var(--gt-text-dim)' }}>
              You need at least {formatMoney(minMinor, currency)} in your balance to request a payout.
            </span>
          )}
        </form>
      )}
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--gt-text-faint)' }}>
                    ...
                    {r.disbursementRef ? ` · reference ${r.disbursementRef}` : ''}
                  </span>
                </div>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {ledger.length === 0 ? (
          ...
          >
            Nothing yet. Digital payments and payouts show up here.
          </div>
        ) : (
          <div style={{ border: '1px solid var(--gt-border)', borderRadius: 12, overflow: 'hidden' }}>
            {ledger.map((entry, i) => {
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                    <span style={{ fontSize: 14 }}>{ENTRY_LABEL[entry.type]}</span>
                    <span style={{ fontSize: 12, color: 'var(--gt-text-faint)' }}>
