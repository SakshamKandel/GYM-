      ...
      {error ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Section title="Deliver to">
            <div style={{ fontSize: 15 }}>{detail.deliveryName}</div>
            <a
              href={`tel:${detail.deliveryPhone.replace(/[^+\d]/g, '')}`}
              style={{ fontSize: 14, color: 'var(--gt-accent-strong)', textDecoration: 'none' }}
            >
              {detail.deliveryPhone}
            </a>
            <div style={{ fontSize: 14, color: 'var(--gt-text-dim)', marginTop: 4 }}>
