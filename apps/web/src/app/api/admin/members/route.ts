 ...
 *
 *  - GET → accounts (id, email, displayName, tier, status). Optional filters:
 *          `?q=` email substring (case-insensitive), `?status=active|suspended`.
 *          Capped at 100 rows so a large member base can't return an unbounded
 *          payload; the UI is expected to narrow with `q`.
 *
