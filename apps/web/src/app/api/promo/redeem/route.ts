 ...
 * for this (account, code) means an earlier redeem bumped redemptionCount but
 * crashed before granting the discount — the retry repairs it by inserting
 * only the missing grant (never re-bumps the counter or touches the
 * redemption row). See the inline comment below for the full reasoning.
 *
...
export async function POST(req: Request) {
  ...
  if (existing.length > 0) {
    ...
    // alone doesn't mean broken. What DOES mean broken: redemptionCount was
    // already bumped (this row exists) but grantDiscount() never ran/committed
    // (e.g. the process died between the two steps) — in that case NO
    // discount_grants row exists for this (account, code) at all, regardless
    // of status. If any such row exists (active, expired-by-a-later-better-
    // grant, or consumed), grantDiscount already succeeded once and this is a
    // genuine repeat redeem attempt. Only the no-row case gets repaired, and
    // the repair only inserts the missing grant — it never re-touches
    // promoRedemptions or bumps redemptionCount again.
